...
最小，达到全局最优。 
* ### 二分K-均值聚类算法
	二分k均值算法是k-means算法的一个变体，主要是为了改进k-means算法随机
选择初始质心的随机性造成可能聚类结果局部最优的问题，而Bisecting k-means算法受随机选择初始质心的影响比较小。
...
将$C_p$分为2个簇$C_{i1}$、$C_{i2}$，一共得到m个二分结果集合$B=\{B_1,B_2,…,B_m\}$，$B_i=\{C_{i1},C_{i2}\}$，
其中$C_{i1}$和$C_{i2}$为每一次二分试验得到的2个簇。  
