...
平均距离：$$dist_avg (C_i,C_j)=/fac{1}{n_i n_j}/sum_{p∈C_i,p∈C_j}|p-p'|$$

层次聚类算法也是基于距离来衡量相似性的，不过距离衡量的是簇的相似性，
层次聚类也是旨在发现球形簇。  
	当使用最小距离来度量两个簇之间的距离时，有时被称为最近邻聚类算法。
当最近的两个簇之间的距离超过设定的阈值之后，迭代就会终止，则其成为单连接算法。  
	当时用最大距离来度量两个簇之间的距离时，有时被称为最远邻聚类算法。
