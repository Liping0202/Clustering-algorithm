...
*  **Canopy算法原理：**  
	...
	（如果当前没有canopy，则该点直接作为canopy）。
	在List中随机挑选一个数据向量A，使用一个粗糙距离计算
	方式计算A与List中其他样本数据向量之间的距离d。	
	(3) 根据(2)得到的距离d，把d小于T1的数据点标上弱标记并划到这个canopy中（同时这个数据可以作为新的canopy来计算其
	他数据到这个点的距离），把d小于T2的数据点标上强标记并从List中移除，此时认为这个数据点距离该canopy已经足够近了，
	不可能在形成新的canopy。  
