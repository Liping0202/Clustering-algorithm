...
* [chapter10 密度聚类法](#chapter10-密度聚类法)  
	* [DBSCAN算法](https://github.com/Liping0202/Clustering-algorithm/tree/master/DBSCAN)
	* [Optics算法](https://github.com/Liping0202/Clustering-algorithm/tree/master/Optics)
* [chapter11 基于网格聚类法](#chapter11-基于网格聚类法)  
	* [基于网格聚类法](#chapter11-基于网格聚类法)
...
$B_i=[C_{i1},C_{i2}]$，其中$C_{i1}$和$C_{i2}$为每一次二分试验得到的2个簇。  
	(3) 计算集合B中每一个划分方法得到的2个簇的总SSE值，选择具有最小总SSE的二分方法得到的结果：$B_j=[C_{j1},C_{j2}]$，
...
当最近的两个簇之间的距离超过设定的阈值之后，迭代就会终止，成为单连接算法。  
	当使用最大距离来度量两个簇之间的距离时，被称为最远邻聚类算法。
如果最近的两个簇之间的最远距离大于某个设定的阈值，迭代终止，称为全连接算法。  
	最小/最大距离代表了簇间距离度量的两个极端，对离群点以及噪声点过分敏感，
...
* ### 传统的层次聚类方法
*  **AGNES算法原理：**
AGNES算法是凝聚的层次聚类算法，如果簇C1中的一个数据点和簇C2中的一个
数据点之间的距离是所有属于不同簇的数据点间欧式距离中最小的，C1和C2可能被合并。  
算法描述：输入包含n个数据点的数据集，终止类簇的数目k。  
(1)将每个对象当成一个初始簇。  
(2)根据两个簇中最近的数据点找到最近的两个簇并合并，生成新的簇的集合。
(3)重复步骤(2),直到达到要求的簇数目。  
算法性能分析：  
(1)简单，但遇到合并点选择困难的情况;(2)一旦一组对象被合并，不能撤销;(3)算法的复杂度为O(n^2)，不适合大数据集。  

*  **DIANA算法原理：**
 DIANA算法属于分裂的层次聚类，先将所有的对象初始化到一个簇中，然后根据一些原则（比如最邻近的最大欧式距离），将该簇分类。
直到到达用户指定的簇数目或者两个簇之间的距离超过了某个阈值。   
算法描述：  
(1)将整个数据集当成一个初始簇。  
...
将层级聚类和其他聚类技术进行集成，形成多阶段聚类。  
	CHAMELEON是一种两阶段聚类法，它采用动态建模来确定一对簇之间的相似度。
第一阶段把点分成很多小的簇；第二阶段根据相近程度合并这些小的簇。第一阶段采用K最邻
近法，即把一个点和它最邻近的K个点连接起来。
...
划分和层次的聚类算法旨在发现球状簇，很难发现任意形状的簇。为了发现任意形状的簇，
可以把簇看做数据空间中被稀疏区域分开的稠密区域。基于密度的聚类算
法假设聚类结构能够通过样本分布的紧密程度确定。通常情况下，密度聚类算法从样本密
度的角度进行考察样本之间的可连接性。基于密度聚类算法的主要特点是：发现任意形状的簇;处理噪声;一次扫描;
需要密度参数作为终止条件。 
* ### DBSCAN算法
...
DBSCAN算法原理：输入数据集、参数半径 $r$、邻域密度阈值$MinPts$  
(1)标记所有数据点为unvisited；  
(2)遍历数据集，对于数据集中的一个随机数据点$p$，标记其为visited，以$p$为中心，
$r$为半径的圆内的数据点的个数为n,若n大于阈值$MinPts$，则$p$为核心对象，否则为噪声点。将所有核心对象
对应的邻域数据点集合加入到结果集合中。如果一个数据点已经属于某个簇，则无需对其进行核心对象判断。  
(3)对于结果数据集中任意两个集合，若一个数据点为一个集合的核心对象，为另一个集合的邻域数据点，则将两个
集合合并。
(4)重复上述过程直到所有数据点都被标记为visited.  
DBSCAN主要算法思想是：(1)找出所有的核心对象.(2)对所有的核心对象进行随机遍历，找出所有的密度可达点.
(3)当所有的核心对象点遍历完之后，剩下的点就是噪声点或者是低密度区域。
 
参数的取值：两个参数的计算均来自经验知识，半径Eps的计算依赖于计算k-距离，
k的取值就是MinPts的取值。根据得到的所有点的k-距离集合E，对集合E进行升序排序后得到k-距离集合E’，
拟合一条排序后的E’集合中k-距离的变化曲线图，通过观察，将急剧发生变化的位置所对应的k-距离的值，
确定为半径Eps的值。如果觉得经验值聚类的结果不满意，可以适当调整Eps和MinPts的值，
经过多次迭代计算对比，选择最合适的参数值。  
算法分析：  
优点：聚类速度快且能够有效处理噪声点和发现任意形状的空间聚类。
缺点：由于直接对整个数据库进行操作且进行聚类时使用了一个全局性的表征密度的参数，
因此当数据量增大时，要求较大的内存支持I/O消耗也很大；
当空间聚类的密度不均匀、聚类间距差相差很大时，聚类质量较差。
并且与用户输入的邻域半径及密度域值密切相关，参数敏感。  
  

DBSCAN算法进行聚类时使用了一个全局性的表征密度的参数，当空间聚类的
密度不均匀、聚类间距差相差很大时，聚类质量较差。为了克服全局参数这一缺点，引入了OPTICS聚类算法。OPTICS并不显式地产生聚类簇，
...
从这个队列里面可以获得任意密度的聚类。   
*  **OPTICS算法原理：**      
输入：数据集D，半径r，给定点在r邻域内成为核心对象的最小邻域点数MinPts。    
1、建立两个队列，有序队列和结果队列(有序队列存储核心对象及其该核心对象的直接可达对象
，并按照可达距离升序排列。结果队列存储样本点的输出次序)；     
2、如果数据集D中数据全部处理完，则算法结束，否则从D中选择一个未处理（不在结果队
列中）且为核心对象的数据点，将该核心点放入结果队列，该核心点的直接密度可达点放入
有序队列，直接密度可达点按可达距离升序排列；      
...
可达距离较小，则更新该点的可达距离；    
③重复步骤3，直至有序队列为空。  
以流程图的方式来展现算法的执行过程：其中D为待聚类的集合，Q为有序队列，
元素按照可达距离排序，可达距离最小的在队首，O为结果队列，最后输出结果
的点集的有序队列。
##### Optics算法的流程图：
![ Optics算法的流程图](https://github.com/Liping0202/Clustering-algorithm/blob/master/Optics.png) 
得到结果队列后，使用如下算法得到最终的聚类结果：  
(1)从结果队列中按顺序取出点，如果该点的可达距离不大于给定半径r，则该点属于
当前类别，否则至步骤(2)。
...
算法得到的簇排序信息等价于一个广泛的参数设置所获得的基于密度的聚类结果。之所以
需要给定这个半径r，是因为基于密度的聚类算法需要确定哪些点是核心点，
哪些是躁点，所以必须需要给定半径r。     
...
基于网络聚类方法原理就是将数据空间划分为网格单元，将数据对象集映射到网格单元中，
并计算每个单元的密度。根据预设的阈值判断每个网格单元是否为高密度单元，由邻近的
稠密单元组形成"类"。 
* ### 基于网格聚类法算法流程  
用不同的网格划分方法，将数据空间划分成为有限个单元的网格结构,并对网格数据结构
进行了不同的处理，但核心步骤是相同的:  
(1)划分网格  
...
(4)最后将相连的高密度网格单元识别为簇  
算法优缺点  
优点：速度很快，因为其速度与数据对象的个数无关，而只依赖于数据空间中每个维上
单元的个数。缺点：参数敏感、无法处理不规则分布的数据、维数灾难等；这种算法效率的提高是
以聚类结果的精确性为代价的。   
常见的算法及改进:STING算法、WAVE-CLUSTER算法和CLIQUE算法。
